...
mod tests {
    ...
    fn run(input: &str) -> (Value, Vec<String>) {
        ...
        let result = vm.run().expect("実行時エラー");
        (result, vm.output)
    }

    #[test]
    fn test_print_string() {
        let (_, output) = run("「こんにちは世界」と 表示する");
        assert_eq!(output, vec!["こんにちは世界"]);
    }

    #[test]
    fn test_binding_and_print() {
        let (_, output) = run("名前 は 「太郎」\n名前と 表示する");
        assert_eq!(output, vec!["太郎"]);
    }

    #[test]
    fn test_arithmetic() {
        let (_, output) = run("x は 3\ny は 4\nxとyの和と 表示する");
    ...
    fn test_list_literal() {
        let (_, output) = run("一覧 は 【1、2、3】\n一覧と 表示する");
        assert_eq!(output, vec!["【1、2、3】"]);
    }

    #[test]
    fn test_string_concat() {
        let (_, output) = run("「hello」と「world」の和と 表示する");
        assert_eq!(output, vec!["helloworld"]);
    }

    #[test]
    fn test_boolean_logic() {
        let (_, output) = run("もし 真 かつ 真 ならば\n  「両方真」と 表示する");
        assert_eq!(output, vec!["両方真"]);
    }
}
