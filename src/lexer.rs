...
mod tests {
    ...
    fn test_integer() {
        ...
        assert_eq!(kinds, vec![TokenKind::Integer("42".into())]);
    }

    #[test]
    fn test_fullwidth_integer() {
        let kinds = token_kinds("１２３");
        assert_eq!(kinds, vec![TokenKind::Integer("123".into())]);
    }

    #[test]
    fn test_float() {
        let kinds = token_kinds("3.14");
        assert_eq!(kinds, vec![TokenKind::Float("3.14".into())]);
    }

    #[test]
    fn test_string() {
        let kinds = token_kinds("「こんにちは」");
        assert_eq!(kinds, vec![TokenKind::String("こんにちは".into())]);
    }

    #[test]
    fn test_bool() {
        let kinds = token_kinds("真 偽");
        assert_eq!(kinds, vec![TokenKind::Bool(true), TokenKind::Bool(false)]);
    }

    #[test]
    fn test_particle_attached() {
        let kinds = token_kinds("名前を");
        assert_eq!(
            kinds,
            vec![
    ...
    #[test]
    fn test_particle_standalone() {
        let kinds = token_kinds("名前 を");
        assert_eq!(
            kinds,
            vec![
                TokenKind::Identifier("名前".into()),
    ...
    #[test]
    fn test_particle_long() {
        let kinds = token_kinds("始点から 終点まで");
        assert_eq!(
            kinds,
            vec![
                TokenKind::Identifier("始点".into()),
                TokenKind::Particle(Particle::Kara),
                TokenKind::Identifier("終点".into()),
                TokenKind::Particle(Particle::Made),
            ]
        );
    }
    ...
    #[test]
    fn test_counter() {
        let kinds = token_kinds("5秒");
        assert_eq!(
            kinds,
            vec![
                TokenKind::Integer("5".into()),
                TokenKind::Counter("秒".into()),
            ]
        );
    }
    ...
    #[test]
    fn test_keywords() {
        let kinds = token_kinds("もし ならば そうでなければ");
        assert_eq!(
            kinds,
            vec![
                TokenKind::Moshi,
                TokenKind::Naraba,
                TokenKind::SouDenakereba,
            ]
        );
    }
    ...
    #[test]
    fn test_binding() {
        let kinds = token_kinds("名前 は 「太郎」");
        assert_eq!(
            kinds,
            vec![
                TokenKind::Identifier("名前".into()),
                TokenKind::Ha,
                TokenKind::String("太郎".into()),
            ]
        );
    }
    ...
    fn test_indent_dedent() {
        let kinds = token_kinds("もし 真 ならば\n  表示する");
        assert_eq!(
            kinds,
            vec![
                TokenKind::Moshi,
    ...
    fn test_comment_skipped() {
        let kinds = token_kinds("42 ※ これはコメント");
        assert_eq!(kinds, vec![TokenKind::Integer("42".into())]);
    }
    ...
    #[test]
    fn test_kosoado() {
        let kinds = token_kinds("これ それ あれ こう");
        assert_eq!(
            kinds,
            vec![
                TokenKind::Kore,
    ...
    fn test_procedure_definition() {
        let kinds = token_kinds("挨拶する という 手順【名前:を】");
        assert_eq!(
            kinds,
            vec![
                TokenKind::Identifier("挨拶する".into()),
                TokenKind::ToIu,
                TokenKind::Tejun,
                TokenKind::LBracket,
                TokenKind::Identifier("名前".into()),
    ...
    fn test_logical_operators() {
        let kinds = token_kinds("かつ または でない");
        assert_eq!(
            kinds,
            vec![TokenKind::Katsu, TokenKind::Mataha, TokenKind::DeNai,]
        );
    }

    #[test]
    fn test_string_interpolation() {
        let kinds = token_kinds("「名前は【名前】です」");
        assert_eq!(
            kinds,
            vec![
                TokenKind::String("名前は".into()),
                TokenKind::StringInterpStart,
                TokenKind::Identifier("名前".into()),
                TokenKind::StringInterpEnd,
    ...
    #[test]
    fn test_period_as_separator() {
        let kinds = token_kinds("表示する。表示する");
        assert_eq!(
            kinds,
            vec![
                TokenKind::HyoujiSuru,
